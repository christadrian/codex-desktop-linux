...
function currentAvatarOverlayBundleFixture() {
  return [
    "let a=require(`electron`),f=require(`node:child_process`);function X5(e,t){return e}function r5(e,t){return{shouldDock:!0,anchor:`dock-anchor`,onDock:`dock-handler`}}",
...
test("patches the current dock-threshold drag completion shape", () => {
  const source = currentAvatarOverlayBundleFixture().replace(
    "this.suppressNextRendererThrow=!0;let o=X5(this.anchor,this.presentationOffset),s=r5(o,t);s.shouldDock?this.dockPresentation(s.anchor,s.onDock):this.persistWindowBounds(n,a??this.getCurrentDisplay())",
    "this.suppressNextRendererThrow=!0;let o=this.dockTarget,s=this.anchor;o!=null&&shouldDock({current:s,target:o.anchor}).shouldDock&&this.dockPresentation(o.anchor,o.onDock)",
  );
  const { result, warnings } = captureWarnings(() => applyPetOverlayPatch(source));
