...
function patchNiriDragLifecycle(source) {
  ...
  const completionPattern =
    /([A-Za-z_$][\w$]*)\?this\.persistWindowBounds\(([A-Za-z_$][\w$]*),[A-Za-z_$][\w$]*\?\?this\.getCurrentDisplay\(\)\):this\.reclampWindowToVisibleDisplay\(\{shouldPersist:!0\}\);/;
  ...
  const windowVar = completionMatch[2];
  const completionTail = endMethod.text.slice(completionMatch.index, -1);
  return replaceMethodText(
    patched,
    endMethod,
    `${endMethod.text.slice(0, completionMatch.index)}` +
      `this.codexPetOverlayEndNiriDrag(${windowVar},()=>{${completionTail}})||(()=>{${completionTail}})()` +
      "}",
  );
}
